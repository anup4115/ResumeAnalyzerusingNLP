...
GOOGLE_CLIENT_ID = "795004970875-dtr8iugj3g68geusjm5i2d4h37967lp7.apps.googleusercontent.com"
...
app.config['MONGO_URI']= 'mongodb+srv://admin:admin@cluster0.vrkhtmb.mongodb.net/Database'
...
nlp = spacy.load('C:/Users/ASUS/Desktop/ResumeRankingSystem/assets/ResumeModel/output/model-best')
...
@app.route('/applied_employees',methods=['POST', 'GET'])
def applied_employees():
    # print("Name:",session['user_id'])
    userId = session['user_id']
    user = IRS_USERS.find_one({"_id": ObjectId(userId)}, {"Email": 1})
    ...
    find_status = Applied_EMP.find_one({"user_id":ObjectId(userId)}, {"Status": 1})
    if find_status:
        status = find_status["Status"]
    else:
        # Handle the case when the user is not found
        status = None
    print("status:",status)
