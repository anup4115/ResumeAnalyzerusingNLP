...
def APPLY_JOB():
    ...
    user = IRS_USERS.find_one({"_id": ObjectId(session['user_id']),}, {"Email": 1})
    if user:
        userEmail = user["Email"]
    else:
        userEmail = None
    result = None
    result = Applied_EMP.insert_one({"job_id":ObjectId(job_id),"user_id":ObjectId(session['user_id']),"User_name":session['user_name'],"Email":userEmail,"Status":"pending","Matching_percentage":match_percentage})
    if result == None:
...
def send_email_top5_route():
    ...
    print("Top 5 Emails:",email_list_top5)

    Applied_EMP.update_many(
        {"Email": {"$in": email_list_top5}},
        {"$set": {"Status": "Selected"}}
    )
    
    Applied_EMP.update_many(
        {"Email": {"$nin": email_list_top5}},
